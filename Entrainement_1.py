# -*- coding: utf-8 -*-
"""
Created on Thu Feb 22 20:36:06 2024

@author: alexf
"""

#Si f est une action env.step(f) <-> pm.move(i,j,f)
#env.reset() <-> game_reset()

import jeu_pacman_to_train as pm
...
def simulation(epsilon, debug=False):
  ...
  for i in range(taille_sequence):
    observation, reward, done,jsp, info=env.step(0)
  ...
  while True:
    ...
    if done:
      ...
      return tab_observations,\
             ...
             tab_next_observations,\
             tab_done
    observation, reward, done, jsp, info=env.step(action+1)
    img=transform_img(observation)
    tab_sequence[:-1]=tab_sequence[1:]
    tab_sequence[taille_sequence-1]=img
    if h==0:
