...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar 27 14:33:38 2024
...
#2 méthodes : - la 1 naturelle mais complexité horrible (-> array of int32)
#             - la 2 beaucoup plus rapide               (->array of uint 8)
    if methode==1:
        ...
        img_data = pygame.image.tostring(surface, 'RGB') #bytes
        img = Image.frombytes('RGB', (570,650), img_data)  # le tableau est converti en une matrice rgb numpy
        #plt.rcParams["figure.figsize"] = (15,10)
        #img = Image.fromarray(np.array(img)) le tableau est converti en un objet image spécifique à la bibliothèque PIL
        return np.array(img)
...
def game_reset():
    ...
    img=img_jeu(2)
    sequence_img=(img,img)
    return sequence_img, Coord, Gomme, tour, points, peur, peurs, killstreak, catch, vie


def respawn(fst_img,Gomme,tour,peur,peurs,vie):
    creation_map(map1,Gomme)
    ...
    return sequence_img,Coord, Gomme, tour, points, peur, peurs, killstreak, catch, vie-1
...
def take_action(dp,va_jeu):
    ...
    va_jeu = sequence_img, Coord, Gomme, tour, points, peur, peurs, killstreak, catch, vie
    return va_jeu
...
for p in range(1000):
    ...
    for k in range(1000):
        dp=random.choice([1,2,3,4])
        #dp=random.choice(actions_possibles(Coord[0][0][0],Coord[0][0][1],dp))
        va_jeu = take_action(dp,va_jeu)      
        sequence_img,Coord, Gomme, tour, points, peur, peurs, killstreak, catch, vie = va_jeu
        va_jeu = sequence_img,Coord, Gomme, tour, points, peur, peurs, killstreak, catch, vie
