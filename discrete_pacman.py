...
def bouger_pac_man(i,j,f):
    ...
    if f==1:
        new_coord=(j*30,(i-1)*30)
        surface.blit(pac_man,new_coord)
        return i-1,j
    elif f==2:
        new_coord=((j+1)*30,i*30)
        surface.blit(pac_man,new_coord)
        return i,j+1
    elif f==3:
        new_coord=(j*30,(i+1)*30)
...
def chemin_fantome_rouge_chase (G):
...
def chemin_fantome_rose_chase (G):
    kfrose=Coord[2][0]
    kpm=Coord[0][0]
    (ipm,jpm)=kpm
    d=Coord[0][1]
...
def chemin_fantome_bleu_chase (G):
    (ir,jr)=Coord[1][0]
...
def chemin_fantome_jaune_chase (G):
    ...
    if vg.dist_euclidienne (ij,ij,ik,jk) > 8 :
        ...
        return chemin_fantome_jaune_scatter (G)

def chemin_fantome_rose_scatter (G):
...
def chemin_fantome_rouge_scatter (G):
    kfrouge=Dico[Coord[1][0]]
    if kfrouge != 15 :
...
def chemin_fantome_jaune_scatter (G):
    kfjaune=Dico[Coord[4][0]]
    if kfjaune != 172 :
...
def chemin_fantome_bleu_scatter (G):
    kfbleu=Dico[Coord[3][0]]
    if kfbleu != 188 :
    ...
    for event in pygame.event.get():  
        ...
        if event.type== pygame.KEYDOWN :
            if event.key == pygame.K_a :
                ...
                while not (pac_man_catch()):
                    tour+=1 
                    
                    n= vg.random_except(0,188,Dico[Coord[0][0]])
                    chemin_pm = chemin_pacman(map1, n)
                    ...
                    if state(tour)==0:
                        ...
                        chemin_frouge=chemin_fantome_rouge_chase(map1)
                        d= Direc[Dico[Coord[1][0]]][Dico[chemin_frouge[1]]]
                        bouger_fantome_rouge(Coord[1][0][0], Coord[1][0][1], d)
                        Coord[1][0]=chemin_frouge[1]
                        Coord[1][1]=d
                        
                        chemin_frose=chemin_fantome_rose_chase(map1)
                        d= Direc[Dico[Coord[2][0]]][Dico[chemin_frose[1]]]
                        bouger_fantome_rose(Coord[2][0][0], Coord[2][0][1], d)
                        Coord[2][0]=chemin_frose[1]
                        Coord[2][1]=d
                        
                        chemin_fbleu=chemin_fantome_bleu_chase(map1)
                        d= Direc[Dico[Coord[3][0]]][Dico[chemin_fbleu[1]]]
                        bouger_fantome_bleu(Coord[3][0][0], Coord[3][0][1], d)
                        Coord[3][0]=chemin_fbleu[1]
                        Coord[3][1]=d
                        
                        chemin_fjaune=chemin_fantome_jaune_chase(map1)
                        d= Direc[Dico[Coord[4][0]]][Dico[chemin_fjaune[1]]]
                        bouger_fantome_jaune(Coord[4][0][0], Coord[4][0][1], d)
                        Coord[4][0]=chemin_fjaune[1]
                        Coord[4][1]=d
