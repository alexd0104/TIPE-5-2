...
import Pac_manfinal_classique as fa
...
def model():
  entree=layers.Input(shape=(), dtype='float32')
...
def simulation(epsilon, debug=False):
  ...
  vie=3
  tour=0
  Coord, Gomme, tour, points, peur, peurs, killstreak, catch, new_vie, mort=fa.game_reset()
  state=preprocess_state((Coord, Gomme, tour, points, peur, peurs, killstreak, catch, new_vie, mort))
  ...
  while True:
    ...
    if h==0:
      tab_state.append(np.concatenate(state, axis=-1))
    ...
    if done:
      ...
      if h==0:
        ...
        tab_next_state.append(np.concatenate(state, axis=-1))
    ...
    va_jeu = Coord, Gomme, tour, points, peur, peurs, killstreak, catch, vie, mort 
    state = fa.take_action(action+1, va_jeu)
